	...
	implements Micropolis.Listener, EarthquakeListener
{
	...
	private JToolBar makeToolbar()
	{
		...
		b0.add(makeToolBtn(MicropolisTool.MOVEBLDG));
		...
		b3.add(makeToolBtn(MicropolisTool.PARK));
	...
	private void onToolUp(MouseEvent ev)
	{
		if (toolStroke != null) {
			...
			ToolResult tr;
			if (prevTool == MicropolisTool.MOVEBLDG && phase == 0) {
				tr = toolStroke.applyM(moveInfo);
			}
			else {
				tr = toolStroke.apply();				
			}
